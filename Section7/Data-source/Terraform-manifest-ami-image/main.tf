#Availaibility Zones Datasource | This will list all the AZ for the selected region
...
resource "aws_instance" "ec2instance" {
  ...
  tags = {
    Name = "for_each_data-source.${each.value}" # Or you may use each.key since we have a set of list
  }
}
